...
from utils.util import get_simple_dataset_paths_from_config
...
class HdfsDataset3D(Dataset):
    def __init__(
        self, path: str, points_per_sample: int, size: int
    ):
        self.path = path
...
def _dataloader_simple(
        ...
        renders = get_simple_dataset_paths_from_config(
            args.data_path, args.split_path
        )
        ...
        loader = DataLoader(
            dataset=SimpleDataset(
                renders,
                None,
                args.points_per_sample_in_batch,
                transforms,
            ),
...
def CAD_dataloader(train_split, args) -> DataLoader:
    training = True if train_split=="train" else False
    transforms = get_simple_2d_transforms()
    loader = DataLoader(
        dataset=CADDataset(args.path, train_split, transforms),
        batch_size=args.batch_size,
        shuffle=training,
        drop_last=training,
        num_workers=args.worker)
...
def CAD_train_dataloader(args) -> DataLoader:
    return CAD_dataloader("train", args)

def CAD_valid_dataloader(args) -> DataLoader:
    return CAD_dataloader("valid", args)
