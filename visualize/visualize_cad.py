...
from models.UCSGNet_2d import UCSGNet, CSG_layer
from models.shapeeval import (
    CircleSphereEval,
    ShapeEval,
    SquareCubeEval,
)

OUT_DIR = Path("visualized_results")
...
def aggregate_shape_weights_from_model(
    model: UCSGNet, part_index: int
) -> t.List[np.ndarray]:
    weights = []
    per_layer_out_shapes = model.out_shape_layer * 4
    shapes_per_type = model.num_shape_type
    for index, layer in enumerate(
        model.csg_layer
    ):  # type: (int, RelationLayer)
        param = layer.sampled_shape
...
def visualize_reconstruction_path(
    model: UCSGNet, out_dir: Path, original_img: np.ndarray, prediction: np.ndarray
):
    ...
    for layer_index, relation_layer in enumerate(
        ...
        mask = (
            relation_layer.sampled_shape.detach().cpu().numpy()[0, :, 0]
        )
    ...
    for layer, objects in csg_composer.per_layer_combinations.items():
        for ind, obj in enumerate(objects):
            if obj.used or (
                layer == len(model.csg_layer) and obj.op == OpType.UNION
            ):
                cv2.imwrite(
                    (
                        out_out_dir
...
def visualize_input_and_output_at_given_layer(
    intermediate_results: t.List[torch.Tensor], model: UCSGNet, out_dir: Path
):
    ...
    for layer_index in range(len(intermediate_results)):
        ...
        relation_layer: CSG_layer = model.csg_layer[layer_index]
        mask = (
            relation_layer.sampled_shape.detach().cpu().numpy()[0, :, 0]
        )
...
def visualize_combined_first_layer_results_with_primitives(
    model: UCSGNet, intermediate_results: t.List[torch.Tensor], out_dir: Path
):
    ...
    for output_shape_index in range(first_layer_input_data.shape[-1]):
        ...
        part_index = output_shape_index // model.num_shape_type
        object_index = output_shape_index % model.num_shape_type

        part = model.evaluator.parts[part_index]
        parameters = detach_tensor(part.last_parameters()[0, object_index])
        parameters = np.squeeze(parameters)
        translation = detach_tensor(
            part.shift_vector_prediction()[0, object_index]
        )
        rotation = detach_tensor(
            part.rotation_params_prediction()[0, object_index]
        )

        if isinstance(part, SquareCubeEval):
            base_shape_composer.draw_rectangle(
                parameters, translation, rotation, None
            )
        elif isinstance(part, CircleSphereEval):
...
def visualize_synthetic_dataset(model: UCSGNet, img_path: str):
    loader = DataLoader(
        SimpleDataset(
            [img_path],
...
def visualize_cad_dataset(model: UCSGNet, h5_file_path: str):
    loader = DataLoader(
        CADDataset(
            h5_file_path,
            data_split="valid",
    ...
    for data_index, (image, points, trues, _) in enumerate(tqdm.tqdm(loader)):
        ...
        cv2.imwrite(
            (
                ...
                f"{len(model.csg_layer)}_0_used.png"
            ).as_posix(),
            (detach_tensor(intermediate_prediction)[..., 0] * 255)
            .astype(np.uint8)
            .reshape((ORIGINAL_HEIGHT, ORIGINAL_WIDTH)),
...
def visualize_multiple(
    ...
    if data_type == "synthetic":
        ...
        for sample in tqdm.tqdm(paths):
            visualize_synthetic_dataset(net, os.path.join(data_path, sample))

    elif data_type == "cad":
        visualize_cad_dataset(net, os.path.join(data_path, valid))