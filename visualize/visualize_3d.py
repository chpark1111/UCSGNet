...
from utils.mesh_util import sample_points_polygon
...
class VoxelReconstructor:
    def __init__(self, model: UCSGNet, size: int, sphere_complexity: int):
        self.model = model
        self.size = size
        self.sphere_complexity = sphere_complexity
        self.coords: Optional[torch.Tensor] = None
        self._num_shapes_per_type = model.num_shape_type
    ...
    def retrieve_primitives(
        ...
        for i, (trans, rot) in enumerate(
            ...
            shape_part = self.model.evaluator.parts[type_index]
            if isinstance(shape_part, SquareCubeEval):
                composer.add_box(single_shape_params, trans, rot)
            elif isinstance(shape_part, CircleSphereEval):
    ...
    def reconstruct_single(
        ...
        for vox in voxels:
            ...
            params = [
                ...
                for pair in (
                    self.model.evaluator.get_all_last_predicted_parameters_of_shapes()
                )
            ]
            trans = (
                self.model.evaluator.get_all_shift_vectors()
                .detach()
                .cpu()
                .numpy()
            )
            rot = (
                self.model.evaluator.get_all_rotation_vectors()
                .detach()
                .cpu()
                .numpy()
            )
            sampled_shapes = [
                layer.sampled_shape.detach().cpu().numpy()
                for layer in self.model.csg_layer
            ]
...
def reconstruct_3d(args):
    out_dir = Path("visualized_results") / "3d"
    ...
    reconstructor = VoxelReconstructor(
        net, args.size, args.sphere_complexity
    )
    loader = ShapeNet_valid_dataloader(config)
